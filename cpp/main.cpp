...
int main(int argc, char** argv) {
    ...
    if (argc >= 2 && argv[1][0] != '-') {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            else if (a == "--strong_label_enabled" && need(i)) config.strong_label_enabled = (std::atoi(argv[++i]) != 0);
            else if (a == "--label_object_points" && need(i)) config.label_object_points_path = argv[++i];
            else if (a == "--label_strong_features" && need(i)) config.label_strong_features_path = argv[++i];
            else if (a == "--strong_track_weight" && need(i)) config.strong_track_weight = std::atof(argv[++i]);
            ...
            else if (a == "--strong_label_max_yaw_jump_deg" && need(i)) config.strong_label_max_yaw_jump_deg = std::atof(argv[++i]);
            else if (a == "--strong_label_static_score_tolerance" && need(i)) config.strong_label_static_score_tolerance = std::atof(argv[++i]);
